...
### [Telecom Customer Churn & New-Customer Risk Engine](https://github.com/haniRezaei/Churn-Analysis)

* **The Problem:** High baseline customer churn (~27%) causing multi-million dollar revenue losses, coupled with a lack of early-warning risk detection for new accounts.
* **Solution:** A leakage-free Machine Learning pipeline (splitting data *prior* to encoding) integrated with an automated SQL Server backend (`churn11`) and an interactive Power BI dashboard tracking demographic, service, and regional risk factors.
* **Key Achievement:** Trained a Random Forest model achieving an **AUC of 0.892** with **77% recall on churners**, identifying contract type as the primary churn driver (24.95% feature importance) and uncovering that 93.9% of newly joined customers are at high risk due to month-to-month contract setups.

---

### [Predictive Customer Subscription & Marketing Optimization](https://github.com/haniRezaei/customer-behavior)

* **The Problem:** High acquisition costs and low conversion rates resulting from untargeted, uniform marketing campaigns across membership programs (~3,900 accounts).
* **Solution:** An end-to-end predictive classification workflow utilizing engineered behavioral features (`Spend_Per_Purchase`, `Promo_Engagement`, `Satisfaction_Score`), class balancing via SMOTE, and hyperparameter tuning with 5-fold Stratified K-Fold CV.
* **Key Achievement:** Deployed an **XGBoost probability-scoring engine** achieving **86.5% Test Accuracy** and a **0.916 ROC-AUC**, enabling marketing teams to transition from random outreach to targeted campaigns prioritized by conversion likelihood.

---

### [Retail Sales, Weather & Demographics Driver Analysis](https://github.com/haniRezaei/Retail-Weather-Demographics-Analysis-Retail-Shops)

* **The Problem:** Leadership lacked clarity on the underlying environmental and demographic drivers behind daily sales fluctuations across multi-store locations.
* **Solution:** Merged sales, weather logs, and customer survey data into a single, unified SQL analytical view powering an interactive Power BI report with normalized metrics (`Sales_Per_Customer`) and temporal breakdown hierarchies.
* **Key Achievement:** Isolated environmental and foot-traffic drivers—proving daily revenue closely tracks ambient temperature while rain primarily impacts walk-in volume rather than basket size—and established spend-per-head metrics to optimize cross-store staffing allocation.

---

### [End-to-End Pizza Sales & Revenue Optimization BI](https://github.com/haniRezaei/Pizza-Sales-Analysis-)
